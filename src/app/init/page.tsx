...
import { TabActivateModal } from "@features/tab-activate-modal/ui";
...
export default function InitPage() {
  ...
  const onSubmit = async (data: LoginFormData) => {
    const devModeEnabled = logoClicks > 20 || isDev;
    ...
    } else if (!devModeEnabled && !payload.email && !payload.password) {
    ...
    if (responseData.auth === true) {
      ...
      try {
        if (devModeEnabled) {
          sessionStorage.setItem("foodcort_dev_login", "1");
        } else {
          sessionStorage.removeItem("foodcort_dev_login");
        }
      } catch {}
    } else {
  ...
  return (
    <div className="relative min-h-screen flex flex-col items-center px-6 pt-10 pb-[520px] bg-foreground">
      ...
      />
      {code && <TabActivateModal code={code} showModal={showModal} setShowModal={setShowModal} />}
    </div>
  );
}
